...
public:
    ...
    inline bool is_zero_matrix() const noexcept { return _nnz == size_type(); }

    inline bool empty() const noexcept { return _row_index == nullptr; }

    inline Dimensions dim() const noexcept { return _dim; }

    inline size_type rows() const noexcept { return _dim.rows; }

    inline size_type cols() const noexcept { return _dim.cols; }
            ...
            if (nnz_row(i)) {
                for (size_type j = 0; j < m.rows(); j++) {
                    ...
                    while (posA < _row_index[ i + 1 ] && posB < m._row_index[ j + 1 ]) {
                        ...
                        else if (_col_index[ posA ] < m._col_index[ posB ]) { posA++; }
                        else { posB++; }
                    }
        ...
        for (size_type i = 0; i < rows(); i++) {
            ...
            while (posA < _row_index[ i + 1 ] && posB < other._row_index[ i + 1 ]) {
                if (_col_index[ posA ] == other._col_index[ posB ]) {
                    if (_v[ posA ] + _v[ posB ] != Tp()) out._nnz++;
    ...
    inline CRSMatrix operator-(const CRSMatrix& other) const { return *this + (-other); }
    ...
    void transpose() {
        ...
        auto pos_ptr = new size_type
            [ cols() + 1 ];  // kopia _row_index, pozwala określić odpowiednią pozycję wartości
        pos_ptr[ 0 ] = 0;
...
protected:
    inline size_type ridx_size() const noexcept { return _dim.rows + 1; }
